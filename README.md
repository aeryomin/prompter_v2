...
### Правила формирования имени модели

- В конфиге модели (`config.model_name`) и в колонке `model_name` таблицы `models` хранится **официальное название модели из документации** (например, `Veo 3`, `Kling AI`), без добавления даты/времени.
- Дата и время анализа берутся из поля `created_at` и **добавляются только на уровне UI**:
  - в селекторе моделей (`ModelSelector`) имя отображается как `Model name — ДД.ММ.ГГГГ ЧЧ:ММ`;
  - на странице `/models` рядом с `model_name` показывается та же метка времени.
